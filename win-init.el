...
;;;; 斷行設定 https://letoh.wordpress.com/2009/02/12/line-wrap-in-emacs/
...
(custom-set-variables
 ...
 '(org-agenda-files (quote ("D:/inbox/doc/")))
 '(org-capture-templates
   (quote
    (("f" "名言佳句" plain
      (file+headline "D:/inbox/doc/famous_saying.org" "暫存")
      "*** %?")
     ("s" "schedule" entry
      (file+headline "D:/inbox/doc/calendar.org" "schedule")
      "** %?
 SCHEDULED: %t 

")
     ("d" "diary" plain
      (file+datetree+prompt "D:/inbox/doc/diary.org")
      "")
     ("c" "collect everything" entry
      (file "D:/inbox/doc/capture.org")
      "")
     ("w" "web capture" entry
      (file+headline "D:/inbox/doc/capture.org" "網路資料")
      ""))))
 '(org-file-apps
   (quote
    ((auto-mode . emacs)
     ("\\.mm\\'" . default)
     ("\\.mht\\|.html\\|.maff\\|.htm\\'" . "D:/FirefoxPortable/FirefoxPortable.exe %s")
     ("\\.xls\\|.xlsx\\'" . system)
     ("\\.jpg\\|.png\\|.gif\\|.bmp\\'" . "D:/PortableApps/XnView/xnview.exe %s")
     ("\\.pdf\\'" . system)
     ("\\.mp4\\|.m4a\\|.mp3\\|.avi\\|.mkv\\'" . "D:/PortableApps/mpv/mpv.exe %s"))))
...
(custom-set-faces
 ...
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(default ((t (:family "Noto Sans Mono CJK TC Regular" :foundry "outline" :slant normal :weight normal :height 128 :width normal)))))
...
;; __  .__   __. ____    ____   ______    __       __  ___ 
;;   | |  \ |  | \   \  /   /  /  __  \  |  |     |  |/  / 
;;   | |   \|  |  \   \/   /  |  |  |  | |  |     |  '  /  
;;   | |  . `  |   \      /   |  |  |  | |  |     |    <   
;;   | |  |\   |    \    /    |  `--'  | |  `----.|  .  \  
  ...
  (let (
        (mypaths
         '(
           ...
           "D:/PortableApps/FirefoxPortable/"
           "D:/PortableApps/Everything/"
           "D:/PortableApps/ledger_3.1.1_win_bin/"
...
(add-to-list 'org-structure-template-alist
             '("d" "#+NAME: ?\n#+BEGIN_SRC dot :file D:/inbox/media/dot/圖檔名稱.png :exports results\n\n#+END_SRC") ;; dot 語法，圖檔名稱手工改爲想要的檔名
	     )
...
(define-key key-translation-map [?\C-h] [?\C-?]) ;; 因爲部分模式 C-h 是 help 導致無法使用刪除鍵，將鍵位改爲 C-? 來解決。
...
(setq helm-split-window-in-side-p           t ; 使用 helm 功能，不開啓新視窗或關閉其他視窗
      ...
      helm-locate-fuzzy-match               nil ; 全域搜尋 C-c h l 啟動， C-! 暫停/繼續進行實時搜索，大硬盤搜索遲緩時可用。 
...
(global-set-key (kbd "C-;") 'ace-pinyin-jump-char) ;; 跳轉到漢字+英文，若要增加其他快速鍵，則改爲"C-c j c"，避免重複。
...
(add-to-list 'load-path
             (expand-file-name "~/plugins/ledger-mode"))
